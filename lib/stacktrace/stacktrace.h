...
static inline std::string printStacktrace(unsigned int max_frames = 63)
{
//	std::string pad = "[2018-04-01 16:07:59][  ERROR]";
	std::string pad = "                              ";
	
	std::ostringstream out;

	out << pad << "[\033[1mbegin stack trace\033[0m]" << std::endl;
	...
	if(addrlen == 0)
	{
		out << pad << "  <empty, possibly corrupt>" << std::endl;
		return out.str();
	}
		...
		if(begin_name && begin_offset && end_offset && begin_name < begin_offset)
		{
			...
			*end_offset = '\0';
			...
			if (status == 0)
			{
				...
				out << pad << "  "<<symbollist[i]<<" : "<<funcname<<"+"<<begin_offset << std::endl;
			}
			else {
				// demangling failed. Output function name as a C function with no arguments.
				out << pad << "  "<<symbollist[i]<<" : "<<begin_name<<"+"<<begin_offset << std::endl;
			}
		}
		else
		{
			// couldn't parse the line? print the whole line.
			out << pad << "  " << symbollist[i] << std::endl;
		}
	}
	out << pad << "[\033[1mend stack trace\033[0m]" ;
	free(funcname);
	free(symbollist);
	return out.str();
}
