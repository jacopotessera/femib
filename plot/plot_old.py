...
db = MongoClient('localhost', 27017)['testSimulation2']
simCollection = db['sim']
timestepCollection = db['timestep']
id_ = "1"
...
V = []
w = 1
XX, YY = numpy.mgrid[-w:w:40j, -w:w:40j]
XXX = numpy.linspace(-w,w,41)
YYY = numpy.linspace(-w,w,41)
P = []
...
for t in timesteps:
	...
	if t["time"]%(numpy.ceil(TMAX/40)) == 0:
